...
first_run <- readDNAStringSet("Ayoold.fasta")
second_run <- readDNAStringSet("Nigeria.fasta")
...
writeXStringSet(combined_sequences, "Nigeriaupdate.fasta")
...
Ayonew_allelereport <- read_csv("Nigeria_allelereport.csv", 
                                skip = 3)

file_lines <- readLines("Nigeria_allelereport.csv")
...
writeLines(combined_data, "updated_Nigeria_allelereport.csv")

####################### ALLELEREPORT_XL###################

# Read the new CSV file where sequence names need to be updated
allele_report_XL <- read_csv("Nigeria_allelereport_XL.csv")
...
write_csv(allele_report_XL, "updated_Nigeria_allelereport_XL.csv")
